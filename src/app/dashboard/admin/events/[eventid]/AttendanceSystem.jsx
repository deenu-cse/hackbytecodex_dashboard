...
export default function QRAttendanceSystem({ eventId, isSuperAdmin = false }) {
  ...
  const fetchEventAndRegistrations = useCallback(async (page = 1) => {
    try {
      ...
      if (eventData.success) setEvent(eventData.data.event || eventData.data);
  ...
  const stats = {
    ...
    attended: registrations.filter(r => r.attendance?.marked || r.attendance).length,
    paid: registrations.filter(r => r.payment?.status === "PAID").length,
    revenue: registrations.filter(r => r.payment?.status === "PAID")
      .reduce((acc, r) => acc + (r.payment?.amount || 0), 0)
  };
  ...
  const exportCSV = () => {
    ...
    const csvContent = [
      ...
      ...registrations.map(r => [
        ...
        r.status,
        r.payment?.status,
        (r.attendance?.marked || r.attendance) ? "Yes" : "No",
  ...
  const isCheckedIn = (reg) => {
    return reg.attendance?.marked === true || reg.attendance === true;
  };

  const getCheckedInTime = (reg) => {
    return reg.attendance?.markedAt ? new Date(reg.attendance.markedAt).toLocaleTimeString() : null;
  };
                        ...
                        {isCheckedIn(reg) ? (
                          <>
                            ...
                            <span className="text-xs text-green-400">
                              {getCheckedInTime(reg) || "Checked In"}
                            </span>
                          </>
                        ) : (
          ...
          <div className="w-full max-w-2xl rounded-3xl bg-[#0f0f0f] border border-white/10 p-8 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 rounded-2xl bg-white/5 border border-white/10 mb-6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${isCheckedIn(showDetailModal) ? 'bg-green-500' : 'bg-gray-500'}`} />
                  <span className={isCheckedIn(showDetailModal) ? 'text-green-400' : 'text-gray-400'}>
                    {isCheckedIn(showDetailModal) ? 'Checked In' : 'Not Checked In'}
                  </span>
                </div>
                {showDetailModal.attendance?.markedAt && (
