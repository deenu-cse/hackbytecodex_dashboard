...
export default function DashboardPage() {
  const { user, isSuperAdmin, isCoreTeam, isCollegeLead, isClubAdmin, isStudent } = useAuth();
    ...
    const fetchDashboard = async () => {
      try {
        ...
        if (isSuperAdmin) {
          ...
          data = await dashboardApi.getCollegeLeadDashboard();
        } else if (isClubAdmin) {
          data = await dashboardApi.getClubAdminDashboard();
        } else if (isStudent) {
          data = await dashboardApi.getStudentDashboard();
        }
  ...
  }, [user, isSuperAdmin, isCollegeLead, isClubAdmin, isStudent]);
  ...
  const getStats = () => {
    if (!dashboardData) return staticStats;
    
    if (isSuperAdmin) {
      return [
        { name: "Total Colleges", value: dashboardData.overview?.totalColleges || "0", change: "+2", icon: GraduationCap, color: "blue" },
        { name: "Total Clubs", value: dashboardData.overview?.totalClubs || "0", change: "+5", icon: Users, color: "purple" },
        { name: "Total Events", value: dashboardData.overview?.totalEvents || "0", change: "+12", icon: Calendar, color: "yellow" },
        { name: "Total Users", value: dashboardData.overview?.totalUsers || "0", change: "+48", icon: Users, color: "green" },
      ];
    } else if (isCollegeLead) {
      return [
        { name: "Total Clubs", value: dashboardData.overview?.totalClubs || "0", change: "+1", icon: Users, color: "blue" },
        { name: "Total Events", value: dashboardData.overview?.totalEvents || "0", change: "+3", icon: Calendar, color: "purple" },
        { name: "Total Students", value: dashboardData.overview?.totalStudents || "0", change: "+24", icon: Users, color: "yellow" },
        { name: "Registrations", value: dashboardData.overview?.totalRegistrations || "0", change: "+56", icon: TrendingUp, color: "green" },
      ];
    } else if (isClubAdmin) {
      return [
        { name: "Total Members", value: dashboardData.overview?.totalMembers || "0", change: "+8", icon: Users, color: "blue" },
        { name: "Total Events", value: dashboardData.overview?.totalEvents || "0", change: "+2", icon: Calendar, color: "purple" },
        { name: "Upcoming", value: dashboardData.overview?.upcomingEventsCount || "0", change: "+1", icon: TrendingUp, color: "yellow" },
        { name: "Activities", value: dashboardData.recentActivities?.length || "0", change: "+5", icon: Zap, color: "green" },
      ];
    } else {
      // Student
      return [
        { name: "Events Joined", value: dashboardData.stats?.totalParticipated || "0", change: "+2", icon: Calendar, color: "blue" },
        { name: "Projects Built", value: "8", change: "+2", icon: Code2, color: "purple" },
        { name: "Current Rank", value: `#${user?.performance?.rank || "--"}`, change: "+1", icon: Trophy, color: "yellow" },
        { name: "Reward Points", value: (user?.rewards?.points || 0).toLocaleString(), change: "+50", icon: Star, color: "green" },
      ];
    }
  };

  const getRecentEvents = () => {
    if (!dashboardData) return staticRecentEvents;
    
    if (isSuperAdmin) {
      return dashboardData.recentEvents?.slice(0, 3).map(event => ({
        title: event.title,
        date: new Date(event.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
        status: event.status,
        type: event.eventType,
      })) || [];
    } else if (isCollegeLead || isClubAdmin) {
      return dashboardData.upcomingEvents?.slice(0, 3).map(event => ({
        title: event.title,
        date: new Date(event.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
        status: "Upcoming",
        type: event.eventType,
      ...
      return dashboardData.registeredEvents?.slice(0, 3).map(reg => ({
        ...
        status: reg.attendance ? "Completed" : "Registered",
        type: reg.event?.eventType || "Event",
      })) || [];
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400">
            {isSuperAdmin && "Here's what's happening across the platform"}
            {isCollegeLead && "Here's your college overview"}
            {isClubAdmin && "Here's your club performance"}
            {isStudent && "Here's what's happening in your network"}
          </p>
        </div>
      ...
      {loading && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map(i => (
            <Card key={i} className="bg-[#0f0f0f] border-white/10 animate-pulse h-32" />
          ))}
        </div>
      )}
      ...
      {/* Stats Cards - Only show when not loading and no error */}
      {!loading && !error && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
